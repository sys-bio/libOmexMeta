...
\begin{tikzpicture}[->,>=stealth',node distance=1cm]
  ...
  \node[block]
    (blood) at (10,0) {\href{http://identifiers.org/fma/FMA:9670}{Blood}};
  \node[block,right = 2.5cm of blood]
    (left-ventricle) {\href{http://identifiers.org/fma/FMA:9466}{Left ventricle}};

  \path[-{Latex[length=3mm]}] (blood.east) edge [out=0,in=180]
    node [label={\href{http://co.mbine.org/standards/qualifiers}{isPartOf}}] {}
    node [label=below:(Struct rel.)] {} (left-ventricle.west);
