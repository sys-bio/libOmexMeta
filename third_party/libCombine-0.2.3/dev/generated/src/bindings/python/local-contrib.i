 ...
 * @brief   Contributed class that makes using libCombine from python more convenient
 *          for the time being this only works for python 2.x, but won't break python 3
 * @author  Gordon Ball
 * 
 *<!---------------------------------------------------------------------------
 * This file is part of libCombine.  Please visit http://omex.org for more
 * information about Ca, and the latest version of libCombine.
 *
 ...
 * and also available online as http://omex.org/software/libcombine/license.html
 *----------------------------------------------------------------------- -->*/

 
/**
 * Metaclass to convert getX/setX methods into python properties.
 * This is attached to CaBase and automatically applies for all classes
    ...
    def __new__(cls, classname, bases, classdict):
        ...
        for name in get_methods | set_methods:
            ...
            if name in get_methods:
                ...
                #test if function is "return _libcombine.CLASS_getNAME(__args__)"
                if getter.func_code.co_names == ('_libcombine', cname):
                    getter = getattr(_libcombine, cname)
            ...
            if name in set_methods:
                ...
                if setter.func_code.co_names == ('_libcombine', cname):
                    setter = getattr(_libcombine, cname)
            ...
            if 'unset' + name in classdict:
                ...
                if deleter.func_code.co_names == ('_libcombine', cname):
                    deleter = getattr(_libcombine, cname)
