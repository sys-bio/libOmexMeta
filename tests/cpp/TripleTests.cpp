...
public:
    ...
    TripleTests() {
        this->subject = semsim::Subject(world_, semsim::RDFURINode(world_.newNodeUriString(subject_str)));
        this->resource = semsim::Resource(world_, semsim::RDFURINode(
                world_.newNodeUriString(resource_namespace + "/" + resource_id)));
...
TEST_F(TripleTests, TestPredicate2) {
    ...
    const char *actual = (const char *) librdf_uri_as_string(
            librdf_node_get_uri(*triple.getPredicatePtr()->getNode().getNode()));
    ASSERT_STREQ(expected.c_str(), triple.getPredicatePtr()->str().c_str());
}
...
TEST_F(TripleTests, TestTripleVecGetResource) {
    ...
    std::string actual = (const char *) librdf_uri_to_string(
            librdf_node_get_uri(*vec[0].getResource().getNode().getNode()));
...
TEST_F(TripleTests, TestToStatementSubject) {
    ...
    auto x = librdf_statement_get_subject(*statement.getStatement()); // anonymous struct
...
TEST_F(TripleTests, TestToStatementPrediacte) {
    semsim::Triple triple1(world_, subject, predicatePtr, resource);
    semsim::LibrdfStatement statement = triple1.toStatement();
    auto x = librdf_statement_get_predicate(*statement.getStatement()); // anonymous struct
    std::string actual = (const char *) librdf_uri_as_string(x->value.uri);
