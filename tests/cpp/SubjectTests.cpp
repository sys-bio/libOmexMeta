...
TEST_F(SubjectTests, TestSubjectFromUri) {
    Subject subject = Subject::fromRawPointer(LibrdfNode::fromUriString("meta754"));
...
TEST_F(SubjectTests, TestStringUri) {
    ...
    Subject subject = Subject::fromRawPointer(LibrdfNode::fromUriString(url_str));
...
TEST_F(SubjectTests, TestGetNodeBlank) {
    ...
    Subject subject = Subject::fromRawPointer(LibrdfNode::fromUriString(string));
    ASSERT_STREQ(string.c_str(), subject.getNode().str().c_str());
}

TEST_F(SubjectTests, TestGetNodeAsConstRef) {
    std::string string = "blank";
    Subject subject = Subject::fromRawPointer(LibrdfNode::fromUriString("blank"));
