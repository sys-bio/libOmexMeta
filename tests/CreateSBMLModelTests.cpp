...
TEST(CreateSBMLModelTests, test1) {
    // create SBML model
<<<<<<< HEAD
    auto *doc = new SBMLDocument(3, 2);
    Model *model = doc->createModel("ATestModel");
    ...
    p->setValue(1);
    k->setMath(SBML_parseL3FormulaWithModel("glucose_import_rate", model));
    ...
    std::cerr << sbml_writer.writeSBMLToString(doc) << "\n";
    ...
    SBMLModel &model2 = importer.getSBMLModel();
    ...
    std::cerr << model2.getRDF("./mymodel.xml", "turtle") << "\n";
    ...
    Component *c = model2.getComponentForId("cytosol");
    ...
    std::cerr << model2.humanize() << "\n";

    exportToOmex(model2, "/tmp/combine.omex");
=======
    SBMLDocument doc(3, 2);
    SBMLDocument* docPtr = &doc;
