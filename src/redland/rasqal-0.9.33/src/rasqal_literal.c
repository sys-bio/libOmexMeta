...
rasqal_literal_set_typed_value(rasqal_literal *l, rasqal_literal_type type,
    ...
    switch (type) {
         ...
         */
                l->string = RASQAL_GOOD_CAST(unsigned char*,
                                             rasqal_xsd_decimal_as_counted_string(l->value.decimal, &slen));
                l->string_len = RASQAL_BAD_CAST(unsigned int, slen);
                if (!l->string)
                    return 1;
            }
        ...
        case RASQAL_LITERAL_DATETIME:
            if (1) {
                ...
                RASQAL_FREE(char*, l->string);
                l->string = RASQAL_GOOD_CAST(unsigned char*,
                                             rasqal_xsd_datetime_to_counted_string(l->value.datetime, &slen));
                l->string_len = RASQAL_BAD_CAST(unsigned int, slen);
                if (!l->string)
                    return 1;
            }
...
rasqal_literal_ebv(rasqal_literal *l) {
    ...
    if (l->type == RASQAL_LITERAL_BOOLEAN && !l->value.integer) {
        ...
        b = 0;
            ...
            ((l->type == RASQAL_LITERAL_INTEGER || l->type == RASQAL_LITERAL_INTEGER_SUBTYPE) && !l->value.integer) ||
