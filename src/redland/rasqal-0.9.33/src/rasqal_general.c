...
rasqal_escaped_name_to_utf8_string(const unsigned char *src, size_t len,
    ...
    while (len > 0) {
        ...
        if (c > 0x7f) {
            ...
            size_t unichar_len = RASQAL_GOOD_CAST(size_t, raptor_unicode_utf8_string_get_char(
                    RASQAL_GOOD_CAST(const unsigned char*, p), len + 1, NULL));
            if (unichar_len > len) {
                if (error_handler)
...
rasqal_uri_init(rasqal_world *world) {
    ...
    world->rdf_first_uri = raptor_new_uri_from_uri_local_name(world->raptor_world_ptr, world->rdf_namespace_uri,
                                                              RASQAL_GOOD_CAST(const unsigned char*, "first"));
    world->rdf_rest_uri = raptor_new_uri_from_uri_local_name(world->raptor_world_ptr, world->rdf_namespace_uri,
                                                             RASQAL_GOOD_CAST(const unsigned char*, "rest"));
    world->rdf_nil_uri = raptor_new_uri_from_uri_local_name(world->raptor_world_ptr, world->rdf_namespace_uri,
                                                            RASQAL_GOOD_CAST(const unsigned char*, "nil"));
