...
static int rasqal_query_results_write_sparql_xml(rasqal_query_results_formatter *formatter, raptor_iostream *iostr,
                                                 rasqal_query_results *results, raptor_uri *base_uri);

static rasqal_rowsource *
rasqal_query_results_get_rowsource_sparql_xml(rasqal_query_results_formatter *formatter, rasqal_world *world,
                                              rasqal_variables_table *vars_table, raptor_iostream *iostr,
            ...
            if (!l) {
                ...
                switch (l->type) {
                    case RASQAL_LITERAL_URI:
                        ...
                        raptor_xml_writer_start_element(xml_writer, element1);
                        raptor_xml_writer_cdata(xml_writer, RASQAL_GOOD_CAST(const unsigned char*,
...
rasqal_sparql_xml_sax2_start_element_handler(void *user_data,
    ...
    switch (state) {
        ...
        case STATE_binding:
            con->result_offset = rasqal_rowsource_get_variable_offset_by_name(con->rowsource,
                                                                              RASQAL_GOOD_CAST(const unsigned char*,
                                                                                               con->name));
            break;
    ...
    switch (con->state) {
        ...
        case STATE_literal:
            if (1) {
                ...
                if (con->datatype)
                    datatype_uri = raptor_new_uri(con->world->raptor_world_ptr,
                                                  RASQAL_GOOD_CAST(const unsigned char*, con->datatype));
                if (con->language) {
...
rasqal_query_results_get_rowsource_sparql_xml(rasqal_query_results_formatter *formatter,
    ...
    con->results_sequence = raptor_new_sequence((raptor_data_free_handler) rasqal_free_row,
