...
sub format_notes($$) {
    ...
    $notes = ~s{
#
            ((?:raptor|librdf|rasqal)\w+)}
    { format_type_name_as_docbook_xml($1) }
    ge;
    $notes = ~s{
#
            ?((?:RAPTOR|LIBRDF|RASQAL)_\w+)}
    { format_enum_name_as_docbook_xml($1) }
    ge;
    $notes = ~s{((?:raptor|librdf|rasqal)_\w+?)\(}
    {
        format_function_name_as_docbook_xml($1).
        "("
    }
    ge;
...
@) {
my($fh, $title, $format_name, $show_sig,
@list)=@
_;
...
my $item(
@list) {
my($fn_return, $fn_name, $fn_args, $notes
) = @
$item;
...
print $fh
"    <listitem><para>$formatted_fn $notes</para></listitem>\n";
}
print $fh
<<"EOT";
</itemizedlist>
...
@) {
my($fh, $title, $old_function_header, $new_function_header,
@list)=@
_;
...
<tbody>
<tr>
<th> $old_function_header<
/th>
<th> $new_function_header<
/th>
<th> Notes<
/th>
</tr>
...
my($old_fn_return, $old_fn_name, $old_fn_args,
        $new_fn_return, $new_fn_name, $new_fn_args, $notes
) = @
$item;
...
@) {
my($fh, $title, $old_type_header, $new_type_header,
@list)=@
_;
...
my $item(
@list) {
my($old_type_name, $new_type_name, $notes
) = @
$item;
...
@) {
my($fh, $title, $old_enum_header, $new_enum_header,
@list)=@
_;
...
print $out_fh
qq{
s|^(.*$name.*)\$|/\\* WARNING: $name - deleted. $note \\*/ \$1|
g;
\n};
}
}
...
# the replacement for subject and object can only be determined by a person
    my( % statement_field_maps) = (
                                          'subject' = >
                                  'subject.value.uri or subject.value.blank.string /* WARNING: must choose one */',
...
open(IN,
"<$file") or die "$program: Cannot read $file - $!\n";
while(<IN>) {
chomp;
...
my(@fields)=split(/\t/);
my $actual_n_fields = scalar(
@fields);
die "$program: Bad line has $actual_n_fields fields expected $expected_n_fields $.: $_\n"
unless $actual_n_fields
==
$expected_n_fields;

if($fields[1] eq 'type') {
my($old_ver, $dummy1, $old_name, $old_args, $new_ver, $dummy2, $new_name, $new_args, $notes
)=@
fields;

my $version_pair = $old_ver.
"-".
$new_ver;
if(!$version_pairs_seen{
$version_pair}) {
...
push(@{
$new_types{
$version_pair}}, [$new_name, $notes]);
}
elsif($new_name
eq '-') {
push(@{
$deleted_types{
$version_pair}}, [$old_name, $notes]);
}
elsif(($old_name
eq $new_name
) &&
...
push(@{
$changed_types{
$version_pair}}, [$old_name, $new_name, $notes]);
}
...
my($old_ver, $old_return, $old_name, $old_args, $new_ver, $new_return, $new_name, $new_args, $notes
)=@
fields;
...
push(@{
$new_functions{
$version_pair}}, [$new_return, $new_name, $new_args, $notes]);
}
elsif($new_name
eq '-') {
push(@{
$deleted_functions{
$version_pair}}, [$old_return, $old_name, $old_args, $notes]);
}
elsif($old_return
eq $new_return
&&
...
push(@{
$changed_functions{
$version_pair}}, [$old_return, $old_name, $old_args, $new_return, $new_name, $new_args, $notes]);
}
}
}
close(IN);
...
between
        versions
including additions, deletions, renames(retaining the same
number of parameters, types and
return
value type
) and
more complex
...
for
my $vp(sort{version_for_sort($b->[1]) cmp version_for_sort($a->[1])}
@version_pairs) {
my($old_version, $new_version
)= @
$vp;
my $id = to_id($old_version).
"-to-" .
to_id($new_version);

my $version_pair = $old_version.
"-".
$new_version;
...
print $out_fh
"# Perl script to upgrade $package $old_version to $new_version\n\n";
...
print_deletes_as_perl_script($out_fh,
'Deleted functions',
(map {
[ $_->[1], $_->[3]] } @f));
...
$renamed_enums{
$version_pair} || []};
print_renames_as_perl_script($out_fh,
'Renamed functions', 1,
@f);
...
$changed_types{
$version_pair} || []};
print_changes_as_perl_script($out_fh,
'Changed functions',
(map {
[ $_->[1], $_->[4], $_->[6]] } @f));
...
DESCRIPTION
        ...
        Turn
a package
's changes TSV file into docbook XML.
