...
main(int argc, char *argv[]) {
    ...
    if (help) {
        ...
        for (i = 0; 1; i++) {
            ...
            desc = rasqal_world_get_query_language_description(world, RASQAL_GOOD_CAST(
            unsigned int, i));
            if (!desc)
                break;
        ...
        puts(HELP_TEXT("F", "data-format NAME     ",
                       "Set the data source format NAME (default: " DEFAULT_DATA_FORMAT_NAME_GRAPH ")"));
        puts(HELP_TEXT("h", "help                 ", "Print this help, then exit"));
        puts(HELP_TEXT("Q URI", "query-base-uri URI", "Set the base URI for the query"));
        puts(HELP_TEXT("R", "result-format NAME   ",
                       "Set the result format NAME (default: " DEFAULT_RESULT_FORMAT_NAME ")"));
        puts("    For variable bindings and boolean results:");

        for (i = 0; 1; i++) {
            const raptor_syntax_description *desc;

            desc = rasqal_world_get_query_results_format_description(world, RASQAL_GOOD_CAST(
            unsigned int, i));
            if (!desc)
                break;
        ...
        for (i = 0; 1; i++) {
            const raptor_syntax_description *desc;
            desc = raptor_world_get_parser_description(raptor_world_ptr, RASQAL_GOOD_CAST(
            unsigned int, i));
            if (!desc)
                break;
