...
print_help(rasqal_world *world, raptor_world *raptor_world_ptr) {
    ...
    puts(HELP_TEXT("f FEATURE(=VALUE)", "feature FEATURE(=VALUE)",
                   HELP_PAD "Set query features" HELP_PAD "  Use `-f help' for a list of valid features"));
...
main(int argc, char *argv[]) {
        ...
        switch (c) {
                ...
                if (optarg) {
                    if (!strcmp(optarg, "help")) {
                        ...
                        if (query_feature_value < 0 && !query_feature_string_value) {
                            fprintf(stderr,
                                    "%s: invalid argument `%s' for `" HELP_ARG(f, feature) "'\nTry '%s " HELP_ARG(f,
                                                                                                                  feature) " help' for a list of valid features\n",
            ...
            case 'F':
                if (optarg) {
                    if (!raptor_world_is_parser_name(raptor_world_ptr, optarg)) {
                        fprintf(stderr,
                                "%s: invalid parser name `%s' for `" HELP_ARG(F,
                ...
                if (optarg) {
                    if (!strcmp(optarg, "simple"))
                        ...
                        if (!rasqal_query_results_formats_check2(world, optarg,
                            ...
                            fprintf(stderr,
                                    "%s: invalid output result format `%s' for `" HELP_ARG(r,
            ...
            case 'R':
                if (optarg) {
                    if (!rasqal_query_results_formats_check2(world, optarg,
                                                             ...
                                                             RASQAL_QUERY_RESULTS_FORMAT_FLAG_READER)) {
                        fprintf(stderr,
                                "%s: invalid input result format `%s' for `" HELP_ARG(R, results - input -
                                                                                         format)  "'\nTry '%s -h' for a list of valid formats\n",
                                program, optarg, program);
                        usage = 1;
                    } else
            ...
            case 'W':
                if (optarg)
                    ...
                    warning_level = 0;
                if (warning_level >= 0)
                    rasqal_world_set_warning_level(world, RASQAL_GOOD_CAST(
    ...
    if (usage) {
        if (usage > 1) {
            fputs(title_string, stderr);
            fputs(rasqal_version_string, stderr);
            putc('\n', stderr);
    ...
    switch (mode) {
        ...
        case MODE_CALL_PROTOCOL_URI:
            if (!quiet) {
                ...
                fputs(RASQAL_GOOD_CAST(
                const char*, service_uri_string), stderr);
            ...
            if (!quiet) {
                ...
                if (mode == MODE_EXEC_QUERY_STRING) {
                    ...
                    if (filename) {
                        ...
                        fputs(RASQAL_GOOD_CAST(
                        const char*, uri_string), stderr);
                    }
                }
                if (base_uri_string) {
                    fputs(" with base URI ", stderr);
                    fputs(RASQAL_GOOD_CAST(
                    const char*, base_uri_string), stderr);
                }
                fputc('\n', stderr);
            }
