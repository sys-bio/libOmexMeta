...
static void raptor_rdfxml_generate_statement(raptor_parser *rdf_parser, raptor_term *subject, raptor_uri *predicate_uri,
                                             raptor_term *object, raptor_term *reified,
...
raptor_rdfxml_parse_init(raptor_parser *rdf_parser, const char *name) {
    ...
    RAPTOR_DAML_List_URI(rdf_xml_parser) = raptor_new_uri_from_uri_local_name(world, RAPTOR_DAML_NS_URI(rdf_xml_parser),
                                                                              (const unsigned char *) "List");
    RAPTOR_DAML_first_URI(rdf_xml_parser) = raptor_new_uri_from_uri_local_name(world,
                                                                               RAPTOR_DAML_NS_URI(rdf_xml_parser),
                                                                               (const unsigned char *) "first");
    RAPTOR_DAML_rest_URI(rdf_xml_parser) = raptor_new_uri_from_uri_local_name(world, RAPTOR_DAML_NS_URI(rdf_xml_parser),
                                                                              (const unsigned char *) "rest");
    RAPTOR_DAML_nil_URI(rdf_xml_parser) = raptor_new_uri_from_uri_local_name(world, RAPTOR_DAML_NS_URI(rdf_xml_parser),
        ...
        switch (state) {
            ...
            case RAPTOR_STATE_PARSETYPE_COLLECTION:
                ...
                if ((element->rdf_attr[RDF_NS_ID] != NULL) +
                    ...
                    raptor_rdfxml_update_document_locator(rdf_parser);
                    raptor_parser_error(rdf_parser,
                                        "Multiple attributes of rdf:ID, rdf:about and rdf:nodeID on element '%s' - only one allowed.",
                                        el_name);
                }
                ...
                if (element->parent) {
                    ...
                    if (element->content_type == RAPTOR_RDFXML_ELEMENT_CONTENT_TYPE_COLLECTION ||
                        ...
                        predicate_uri = (element->content_type == RAPTOR_RDFXML_ELEMENT_CONTENT_TYPE_DAML_COLLECTION)
                                        ? RAPTOR_DAML_first_URI(rdf_xml_parser) : RAPTOR_RDF_first_URI(
                                                rdf_parser->world);
                        ...
                        if (!element->parent->tail_id) {
                            ...
                            predicate_uri = (element->content_type ==
                                             RAPTOR_RDFXML_ELEMENT_CONTENT_TYPE_DAML_COLLECTION) ? RAPTOR_DAML_rest_URI(
                                                    rdf_xml_parser) : RAPTOR_RDF_rest_URI(rdf_parser->world);
            ...
            case RAPTOR_STATE_PROPERTYELT:
                ...
                if (element->rdf_attr[RDF_NS_bagID]) {
                    ...
                    if (RAPTOR_OPTIONS_GET_NUMERIC(rdf_parser, RAPTOR_OPTION_ALLOW_BAGID)) {
                        ...
                        if (element->rdf_attr[RDF_NS_resource] ||
                            ...
                            raptor_parser_error(rdf_parser,
                                                "rdf:bagID is forbidden on property element '%s' with an rdf:resource or rdf:parseType attribute.",
                                                el_name);
                ...
                if (element->rdf_attr[RDF_NS_parseType]) {
                    ...
                    if (raptor_rdfxml_element_has_property_attributes(element)) {
                        raptor_parser_error(rdf_parser, "Property attributes cannot be used with rdf:parseType='%s'",
                                            parse_type);
                        state = RAPTOR_STATE_SKIPPING;
                        element->child_state = RAPTOR_STATE_SKIPPING;
                        finished = 1;
                    ...
                    for (i = 0; i <= RDF_NS_LAST; i++)
                        if (element->rdf_attr[i] && i != RDF_NS_parseType) {
                            raptor_parser_error(rdf_parser, "Attribute '%s' cannot be used with rdf:parseType='%s'",
                                                raptor_rdf_ns_terms_info[i].name, parse_type);
                            state = RAPTOR_STATE_SKIPPING;
                            element->child_state = RAPTOR_STATE_SKIPPING;
                            break;
                        }
                    ...
                    if (!strcmp((char *) parse_type, "Literal"))
                        ...
                        if (RAPTOR_OPTIONS_GET_NUMERIC(rdf_parser, RAPTOR_OPTION_ALLOW_OTHER_PARSETYPES) &&
                            ...
                            if (RAPTOR_OPTIONS_GET_NUMERIC(rdf_parser, RAPTOR_OPTION_WARN_OTHER_PARSETYPES)) {
                                raptor_parser_warning(rdf_parser, "Unknown rdf:parseType value '%s' taken as 'Literal'",
                                                      parse_type);
                            }
                            is_parseType_Literal = 1;
                        }
            ...
            case RAPTOR_STATE_PARSETYPE_RESOURCE:
                ...
                if (state == RAPTOR_STATE_NODE_ELEMENT &&
                    ...
                    if (element_in_rdf_ns &&
                        ...
                        ordinal_predicate_uri = raptor_new_uri_from_rdf_ordinal(rdf_parser->world,
                                                                                element->parent->last_ordinal);
                        ...
                        if (!element->object) {
                            if (element->rdf_attr[RDF_NS_resource]) {
                                ...
                                if (!raptor_valid_xml_ID(rdf_parser,
                                    ...
                                    raptor_parser_error(rdf_parser, "Illegal rdf:nodeID value '%s'",
                                                        (const char *) element->object->value.blank.string);
                                    state = RAPTOR_STATE_SKIPPING;
                                    element->child_state = RAPTOR_STATE_SKIPPING;
                                    break;
                                }
                    ...
                    case RAPTOR_RDFXML_ELEMENT_CONTENT_TYPE_LITERAL:
                        ...
                        if (1) {
                            ...
                            if (element->content_type == RAPTOR_RDFXML_ELEMENT_CONTENT_TYPE_LITERAL) {
                                ...
                                if (!literal_datatype)
                                    literal_language = (unsigned char *) raptor_sax2_inscope_xml_language(
                                            rdf_xml_parser->sax2);
