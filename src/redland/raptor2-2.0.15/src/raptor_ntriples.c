...
raptor_ntriples_parse_term_internal(raptor_world *world,
    ...
    while (*lenp > 0) {
        ...
        if (term_class == RAPTOR_TERM_CLASS_URI && c == ' ') {
            raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator,
                                       "URI error - illegal character %d (0x%02X) found.", c, c);
            return 1;
        }
        ...
        if (c > 0x7f) {
            ...
            if (unichar_len < 0 || RAPTOR_GOOD_CAST(size_t, unichar_len) > *lenp) {
                raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator,
                                           "UTF-8 encoding error at character %d (0x%02X) found.", c, c);
        ...
        switch (c) {
            ...
            case 't':
                if (term_class == RAPTOR_TERM_CLASS_URI) {
                    raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator,
                                               "URI error - illegal URI escape '\\%c'.", c);
                    return 1;
                }
            ...
            case 'U':
                ...
                if (term_class == RAPTOR_TERM_CLASS_URI &&
                    ...
                    raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator,
                                               "URI error - illegal Unicode escape \\u%04lX in URI.", unichar);
                    break;
                }
    ...
    if (end_char && !end_char_seen) {
        raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator,
                                   "Missing terminating '%c' before end of input.", end_char);
        return 1;
    }
        ...
        case '<':
            ...
            if (1) {
                ...
                if (raptor_uri_uri_string_is_absolute(dest) <= 0) {
                    raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator, "URI '%s' is not absolute.",
                                               dest);
                    goto fail;
                }
                ...
                if (!uri) {
                    raptor_log_error_formatted(world, RAPTOR_LOG_LEVEL_ERROR, locator, "Could not create URI for '%s'",
                                               (const char *) dest);
                    goto fail;
                }
