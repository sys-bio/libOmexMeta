...
raptor_rdfxml_ensure_writen_header(raptor_serializer *serializer,
    ...
    context->rdf_RDF_element = raptor_new_xml_element_from_namespace_local_name(context->rdf_nspace,
    ...
    if (base_uri &&
        ...
        attrs[attrs_count] = raptor_new_qname_from_namespace_local_name(serializer->world, context->xml_nspace,
...
raptor_rdfxml_serialize_statement(raptor_serializer *serializer,
    ...
    if (statement->predicate->type == RAPTOR_TERM_TYPE_URI) {
        ...
        if (!name || (name == uri_string)) {
            raptor_log_error_formatted(serializer->world, RAPTOR_LOG_LEVEL_ERROR, NULL,
                                       "Cannot split predicate URI %s into an XML qname - skipping statement",
                                       uri_string);
    ...
    switch (statement->subject->type) {
        case RAPTOR_TERM_TYPE_BLANK:
            attrs[attrs_count] = raptor_new_qname_from_namespace_local_name(serializer->world, context->rdf_nspace,
        ...
        case RAPTOR_TERM_TYPE_LITERAL:
            raptor_log_error(serializer->world, RAPTOR_LOG_LEVEL_ERROR, NULL,
                             "Cannot serialize a triple with a literal subject\n");
            break;
