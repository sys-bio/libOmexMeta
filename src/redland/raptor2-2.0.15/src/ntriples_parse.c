...
static void
raptor_ntriples_generate_statement(raptor_parser *parser, raptor_term *subject_term, raptor_term *predicate_term,
                                   raptor_term *object_term, raptor_term *graph_term);
...
raptor_ntriples_parse_line(raptor_parser *rdf_parser,
    ...
    for (i = 0; i < MAX_NTRIPLES_TERMS + 1; i++) {
        ...
        if (i == 3) {
            ...
            if (*p != '<' && *p != '_' && *p != '"') {
                raptor_parser_error(rdf_parser, "Saw '%c', expected object term <URIref>, _:bnodeID or \"literal\"",
                                    *p);
                goto cleanup;
            }
        } else if (i == 1) {
