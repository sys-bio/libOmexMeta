...
raptor_rss_common_init(raptor_world *world) {
    ...
    for (i = 0; i < RAPTOR_RSS_COMMON_SIZE; i++) {
        ...
        if (namespace_uri) {
            world->rss_types_info_uris[i] = raptor_new_uri_from_uri_local_name(world, namespace_uri,
...
raptor_rss_model_init(raptor_world *world, raptor_rss_model *rss_model) {
    ...
    RAPTOR_RSS_RSS_items_URI(rss_model) = raptor_new_uri_relative_to_base(world,
                                                                          world->rss_namespaces_info_uris[RSS1_0_NS],
...
raptor_new_rss_item(raptor_world *world) {
    ...
    item->world = world;
    item->triples = raptor_new_sequence((raptor_data_free_handler) raptor_free_statement,
