...
raptor_rss10_serialize_init(raptor_serializer *serializer, const char *name) {
    ...
    rss_serializer->triples = raptor_new_sequence((raptor_data_free_handler) raptor_free_statement,
                                                  (raptor_data_print_handler) raptor_statement_print);

    rss_serializer->items = raptor_new_sequence((raptor_data_free_handler) raptor_free_rss_item,
                                                (raptor_data_print_handler) NULL);

    rss_serializer->enclosures = raptor_new_sequence((raptor_data_free_handler) raptor_free_rss_item,
                                                     (raptor_data_print_handler) NULL);
...
raptor_rss10_emit_rss_items(raptor_serializer *serializer) {
    ...
    base_uri_copy = base_uri ? raptor_uri_copy(base_uri) : NULL;
    rss_items_predicate = raptor_new_xml_element(
            raptor_qname_copy(serializer->world->rss_fields_info_qnames[RAPTOR_RSS_FIELD_ITEMS]), NULL, base_uri_copy);
...
raptor_rss10_emit_item(raptor_serializer *serializer,
    ...
    for (fi = 0; fi < RAPTOR_RSS_FIELDS_SIZE; fi++) {
        ...
        for (field = item->fields[f]; field; field = field->next) {
            ...
            base_uri_copy = base_uri ? raptor_uri_copy(base_uri) : NULL;
            predicate = raptor_new_xml_element(raptor_qname_copy(serializer->world->rss_fields_info_qnames[f]), NULL,
                                               base_uri_copy);
            ...
            if (is_atom && field->uri) {
                ...
                if (f == RAPTOR_RSS_FIELD_ENCLOSURE && item_type == RAPTOR_RSS_ITEM) {
                    ...
                    if (enclosure_item) {
                        ...
                        if (enclosure_item->fields[RAPTOR_RSS_RDF_ENCLOSURE_TYPE] &&
                            enclosure_item->fields[RAPTOR_RSS_RDF_ENCLOSURE_TYPE]->value) {
                            attrs[attr_count] = raptor_new_qname_from_namespace_local_name(rss_serializer->world,
                        ...
                        if (enclosure_item->fields[RAPTOR_RSS_RDF_ENCLOSURE_LENGTH] &&
                            enclosure_item->fields[RAPTOR_RSS_RDF_ENCLOSURE_LENGTH]->value) {
                            attrs[attr_count] = raptor_new_qname_from_namespace_local_name(rss_serializer->world,
                                                                                           rss_serializer->nspaces[RSS2_0_ENC_NS],
