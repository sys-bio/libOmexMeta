...
sub end_page($) {
    ...
    the Redland
    distribution
    as < tt > demos / demo.pl< / tt > or from
    the
    < a
...
my($subject_string, $predicate_string, $object_string
)=(split(/-->?/, $statement_string, 3));
...
my $subject = ($subject_string
ne '?') ? RDF::Redland::Node->
new_from_uri_string($subject_string)
:
undef;
my $predicate = ($predicate_string
ne '?') ? RDF::Redland::Node->
new_from_uri_string($predicate_string)
:
undef;
my $object;
if(
...
log_action($host, $db,
"Added statement '$statement_string'");
end_page($q);
exit 0;
}
...
log_action($host, $db,
"Querying for statement '$statement_string'");
} else {
my $uri;
my $source_uri;
my $uri_from_form = 0;
...
if(!$parser) {
log_action($host, $db,
"Failed to create RDF parser $parser_string");
...
log_action($host, $db,
"Parsing $uri_label with parser $parser_string");
...
$parser->
parse_into_model($redland_source_uri, $redland_base_uri, $model
);
...
<tr>
<td><
a href = "$subject_query" > $subject_label <
/a></td>
<td><
a href = "$predicate_query" > $predicate_label <
/a></td>
<td> $object_string<
/td>
<td> $from_object<
