...
sub end_page($) {
    ...
    href = "http://librdf.org/" > Redland< / a >
           and the
               < a
    href = "http://librdf.org/docs/pod/RDF/Redland/Parser.html" > RDF::Redland::Parser< / a >
           < a
    ...
    the Redland
    distribution
    as < tt > demos / ntriples.pl< / tt > or from
    the
    < a
...
sub format_node($) {
    ...
    {
        my
        $str = format_literal($node->literal_value).
        '"';
...
log_action($host,
"Parsing N-Triples URI $uri", time);
