...
const lt_dlvtable *get_vtable(lt_user_data data);
...
try_dlopen(lt_dlhandle *phandle, const char *filename, const char *ext,
    ...
    if (ext) {
        attempt = MALLOC(
        char, LT_STRLEN(filename) + LT_STRLEN(ext) + 1);
        if (!attempt)
            return 1;
    ...
    if (!dir) {
        ...
        if (vtable) {
            ...
            archive_name = MALLOC(
            char, strlen(libprefix) + LT_STRLEN(name) + strlen(libext) + 2);
            *phandle = (lt_dlhandle) lt__zalloc(sizeof(struct lt__handle));
...
lt_dlsym(lt_dlhandle place, const char *symbol) {
    ...
    if (lensym + LT_SYMBOL_OVERHEAD < LT_SYMBOL_LENGTH) {
        ...
        sym = MALLOC(
        char, lensym + LT_SYMBOL_OVERHEAD + 1);
        if (!sym) {
