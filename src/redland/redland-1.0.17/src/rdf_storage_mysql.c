...
librdf_storage_mysql_init(librdf_storage *storage, const char *name,
    ...
    if (!context->layout) {
        context->layout = LIBRDF_MALLOC(
        char*, strlen(default_layout) + 1);
    ...
    if (!status) {
        ...
        sprintf(vars_str, "STATEMENTS_NAME='Statements"
        UINT64_T_FMT
        "'",
                context->model);
...
librdf_storage_mysql_merge(librdf_storage *storage) {
  ...
  KEY ObjectSubject (Object,Subject)\
    ...
    query = LIBRDF_MALLOC(
    char*, strlen(create_table_statements) +
...
librdf_storage_mysql_start_bulk(librdf_storage *storage) {
    librdf_storage_mysql_instance *context = (librdf_storage_mysql_instance *) storage->instance;
    char disable_statement_keys[] = "ALTER TABLE Statements"
    UINT64_T_FMT
    " DISABLE KEYS";
    char disable_literal_keys[] = "ALTER TABLE Literals DISABLE KEYS";
    char lock_tables[] = "LOCK TABLES Statements"
    UINT64_T_FMT
    " WRITE, Resources WRITE, Bnodes WRITE, Literals WRITE";
    ...
    query = LIBRDF_MALLOC(
    char*, strlen(disable_statement_keys) + 21);
    if (!query) {
        librdf_storage_mysql_release_handle(storage, handle);
        return 1;
    }
...
librdf_storage_mysql_stop_bulk(librdf_storage *storage) {
    librdf_storage_mysql_instance *context = (librdf_storage_mysql_instance *) storage->instance;
    char enable_statement_keys[] = "ALTER TABLE Statements"
    UINT64_T_FMT
    " ENABLE KEYS";
    ...
    query = LIBRDF_MALLOC(
    char*, strlen(enable_statement_keys) + 21);
    if (!query) {
        librdf_storage_mysql_release_handle(storage, handle);
        return 1;
    }
...
librdf_storage_mysql_context_add_statement_helper(librdf_storage *storage,
    ...
    if (context->transaction_handle) {
        ...
        query = LIBRDF_MALLOC(
        char*, strlen(insert_statement) + 101);
        if (!query) {
            rc = 1;
            goto tidy;
        }
...
librdf_storage_mysql_context_remove_statement(librdf_storage *storage,
    ...
    if (context_node) {
        query = LIBRDF_MALLOC(
        char*, strlen(delete_statement_with_context) + 101);
        if (!query) {
            librdf_storage_mysql_release_handle(storage, handle);
            return 1;
        }
        ...
        query = LIBRDF_MALLOC(
        char*, strlen(delete_statement) + 81);
        if (!query) {
            librdf_storage_mysql_release_handle(storage, handle);
            return 1;
        }
