...
static int librdf_storage_virtuoso_context_contains_statement(librdf_storage *storage, librdf_node *context_node,
...
librdf_storage_virtuoso_find_statements_with_options(librdf_storage *storage, librdf_statement *statement,
                                                     librdf_node *context_node, librdf_hash *options);

/* context functions */
static int librdf_storage_virtuoso_context_add_statement(librdf_storage *storage, librdf_node *context_node,
                                                         librdf_statement *statement);

static int librdf_storage_virtuoso_context_add_statements(librdf_storage *storage, librdf_node *context_node,
                                                          librdf_stream *statement_stream);

static int librdf_storage_virtuoso_context_remove_statement(librdf_storage *storage, librdf_node *context_node,
                                                            librdf_statement *statement);
...
static librdf_stream *
librdf_storage_virtuoso_find_statements_in_context(librdf_storage *storage, librdf_statement *statement,
...
static int librdf_storage_virtuoso_context_add_statement_helper(librdf_storage *storage, librdf_node *context_node,
                                                                librdf_statement *statement);
    ...
    *is_null = 0;
    ...
    if (len == SQL_NULL_DATA) {
        *is_null = 1;
        ...
        pLongData = LIBRDF_MALLOC(
        char*, LIBRDF_GOOD_CAST(size_t, bufsize));
        if (pLongData == NULL) {
            librdf_log(world, 0, LIBRDF_LOG_ERROR, LIBRDF_FROM_STORAGE, NULL,
...
librdf_storage_virtuoso_node2string(librdf_storage *storage, librdf_node *node) {
    ...
    if (type == LIBRDF_NODE_TYPE_RESOURCE) {
        ...
        ret = LIBRDF_MALLOC(
        char*, valuelen + langlen + datatypelen + 8);
        if (!ret)
            goto end;
...
librdf_storage_virtuoso_context2string(librdf_storage *storage,
    ...
    if (node) {
        ...
        ctxt_node = LIBRDF_MALLOC(
        char*, strlen(context->model_name) + 4);
...
librdf_storage_virtuoso_get_handle(librdf_storage *storage) {
    ...
    if (!connection) {
        ...
        connections[context->connections_count] = LIBRDF_CALLOC(librdf_storage_virtuoso_connection * , 1,
                                                                sizeof(librdf_storage_virtuoso_connection));
        ...
        connections[context->connections_count + 1] = LIBRDF_CALLOC(librdf_storage_virtuoso_connection * , 1,
                                                                    sizeof(librdf_storage_virtuoso_connection));
...
librdf_storage_virtuoso_init(librdf_storage *storage, const char *name,
    ...
    context->conn_str = LIBRDF_MALLOC(
    char*, len + 16);
    if (!context->conn_str)
        return 1;

    context->model_name = LIBRDF_MALLOC(
    char*, strlen(name) + 1);
...
librdf_storage_virtuoso_terminate(librdf_storage *storage) {
    ...
    if (context->model_name)
        LIBRDF_FREE(
    char*, (char *) context->model_name);

    if (context->dsn)
        LIBRDF_FREE(
    char*, (char *) context->dsn);

    if (context->database)
        LIBRDF_FREE(
    char*, (char *) context->database);

    if (context->charset)
        LIBRDF_FREE(
    char*, (char *) context->charset);
...
static int
BindCtxt(librdf_storage *storage, librdf_storage_virtuoso_connection *handle, SQLUSMALLINT col, char *data,
         SQLLEN *ind) {
    SQLUINTEGER ulen;
    int rc;
...
librdf_storage_virtuoso_context_contains_statement(librdf_storage *storage,
    ...
    query = LIBRDF_MALLOC(
    char*, strlen(find_statement) + strlen(ctxt_node) + strlen(subject) + strlen(predicate) + strlen(object) + 1);
    if (!query) {
        ret = 0;
        goto end;
    }
