...
librdf_storage_postgresql_hash(librdf_storage *storage, const char *type,
    ...
    LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(string,
    char*, 0);
...
librdf_storage_postgresql_get_handle(librdf_storage *storage) {
    ...
    conninfo = LIBRDF_MALLOC(
    char*,coninfo_size);
    if (conninfo) {
        sprintf(conninfo, coninfo_template, context->host, context->port, context->dbname, context->user,
                context->password);
...
librdf_storage_postgresql_init(librdf_storage *storage, const char *name,
  ...
  PRIMARY KEY (ID)\
    ...
    const char create_model[] = "INSERT INTO Models (ID,Name) VALUES ("
    UINT64_T_FMT
    ",'%s')";
    const char check_model[] = "SELECT 1 FROM Models WHERE ID="
    UINT64_T_FMT
    " AND Name='%s'";
    int status = 0;
    ...
    LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(name,
    char*, 1);
    ...
    if (context->user && !context->dbname) {
        context->dbname = LIBRDF_MALLOC(
        char*, strlen(context->user) + 1);
    ...
    if (!status && (librdf_hash_get_as_boolean(options, "new") > 0)) {
        query = LIBRDF_MALLOC(
        char*, strlen(create_table_statements) + (20 * 1) + 1);
        if (!query) {
            status = 1;
        } else {
            int i;
    ...
    if (!status) {
        escaped_name = LIBRDF_MALLOC(
        char*, strlen(name) * 2 + 1);
        if (!escaped_name)
            status = 1;
        else {
            int error = 0;
...
librdf_storage_postgresql_merge(librdf_storage *storage) {
  ...
  Context numeric(20) NOT NULL\
    ...
    query = LIBRDF_MALLOC(
    char*, strlen(insert_statements) + 50);
    if (!query) {
        PQclear(res);
        librdf_storage_postgresql_release_handle(storage, handle);
        return 1;
...
librdf_storage_postgresql_terminate(librdf_storage *storage) {
    ...
    if (context->password)
        LIBRDF_FREE(
    char*, (char *) context->password);

    if (context->user)
        LIBRDF_FREE(
    char*, (char *) context->user);

    if (context->dbname)
        LIBRDF_FREE(
    char*, (char *) context->dbname);

    if (context->port)
        LIBRDF_FREE(
    char*, (char *) context->port);

    if (context->host)
        LIBRDF_FREE(
    char*, (char *) context->host);
...
librdf_storage_postgresql_size(librdf_storage *storage) {
    librdf_storage_postgresql_instance *context = (librdf_storage_postgresql_instance *) storage->instance;
    char model_size[] = "SELECT COUNT(*) FROM Statements"
    UINT64_T_FMT;
    char *query;
    PGresult *res;
    long count;
...
librdf_storage_postgresql_node_hash(librdf_storage *storage,
    ...
    if (type == LIBRDF_NODE_TYPE_RESOURCE) {
        ...
        if (add) {
            char create_resource[] = "INSERT INTO Resources (ID,URI) VALUES ("
            UINT64_T_FMT
            ",'%s')";
            int add_status = 0;
            char *escaped_uri;

            escaped_uri = LIBRDF_MALLOC(
            char*, nodelen * 2 + 1);
            if (escaped_uri) {
                int error = 0;
                ...
                query = LIBRDF_MALLOC(
                char*, strlen(create_resource) + 20 + nodelen + 1);
                if (query) {
        ...
        nodestring = LIBRDF_MALLOC(
        char*, valuelen + langlen + datatypelen + 3);
        if (!nodestring) {
            librdf_storage_postgresql_release_handle(storage, handle);
            return 0;
        }
        ...
        if (add) {
            char create_literal[] = "INSERT INTO Literals (ID,Value,Language,Datatype) VALUES ("
            UINT64_T_FMT
            ",'%s','%s','%s')";
            int add_status = 0;
            ...
            if (escaped_value) {
                ...
                if (escaped_lang) {
                    ...
                    escaped_datatype = LIBRDF_MALLOC(
                    char*, datatypelen * 2 + 1);
                    if (escaped_datatype) {
                        if (datatype) {
                        ...
                        if (query) {
                            ...
                            if ((res = PQexec(handle, query))) {
                                if (PQresultStatus(res) == PGRES_COMMAND_OK) {
                                    ...
                                    if (0 ==
                                        strncmp("23505", PQresultErrorField(res, PG_DIAG_SQLSTATE), strlen("23505"))) {
                                        /* Don't care about unique key viloations */
                                        add_status = 1;
                                    } else {
                                        librdf_log(storage->world, 0, LIBRDF_LOG_ERROR, LIBRDF_FROM_STORAGE, NULL,
        ...
        if (add) {
            char create_bnode[] = "INSERT INTO Bnodes (ID,Name) VALUES ("
            UINT64_T_FMT
            ",'%s')";
            int add_status = 0;
            char *escaped_name;
            ...
            if (escaped_name) {
                ...
                query = LIBRDF_MALLOC(
                char*, strlen(create_bnode) + 20 + nodelen + 1);
                if (query) {
...
librdf_storage_postgresql_context_add_statement_helper(librdf_storage *storage,
    ...
    librdf_storage_postgresql_instance *context = (librdf_storage_postgresql_instance *) storage->instance;
    char insert_statement[] = "INSERT INTO Statements"
    UINT64_T_FMT
    " (Subject,Predicate,Object,Context) VALUES ("
    UINT64_T_FMT
    ","
    UINT64_T_FMT
    ","
    ...
    if ((handle = librdf_storage_postgresql_get_handle(storage))) {
        ...
        if (subject && predicate && object) {
            ...
            query = LIBRDF_MALLOC(
            char*, strlen(insert_statement) + (20 * 5) + 1);
            if (query) {
...
librdf_storage_postgresql_contains_statement(librdf_storage *storage,
                                             librdf_statement *statement) {
    librdf_storage_postgresql_instance *context = (librdf_storage_postgresql_instance *) storage->instance;
    char find_statement[] = "SELECT 1 FROM Statements"
    UINT64_T_FMT
    " WHERE Subject="
    UINT64_T_FMT
    " AND Predicate="
...
librdf_storage_postgresql_context_remove_statement(librdf_storage *storage,
                                                   ...
                                                   librdf_statement *statement) {
    librdf_storage_postgresql_instance *context = (librdf_storage_postgresql_instance *) storage->instance;
    char delete_statement[] = "DELETE FROM Statements"
    UINT64_T_FMT
    " WHERE Subject="
    UINT64_T_FMT
    " AND Predicate="
    ...
    " AND Object="
    UINT64_T_FMT;
    char delete_statement_with_context[] = "DELETE FROM Statements"
    UINT64_T_FMT
    " WHERE Subject="
    UINT64_T_FMT
    " AND Predicate="
    ...
    if ((handle = librdf_storage_postgresql_get_handle(storage))) {
        ...
        if (subject && predicate && object) {
            ...
            if (context_node) {
                ...
                query = LIBRDF_MALLOC(
                char*, strlen(delete_statement) + (20 * 4) + 1);
                if (query) {
...
librdf_storage_postgresql_context_remove_statements(librdf_storage *storage,
                                                    librdf_node *context_node) {
    librdf_storage_postgresql_instance *context = (librdf_storage_postgresql_instance *) storage->instance;
    char delete_context[] = "DELETE FROM Statements"
    UINT64_T_FMT
    " WHERE Context="
    UINT64_T_FMT;
    char delete_model[] = "DELETE FROM Statements"
    UINT64_T_FMT;
    u64 ctxt = 0;
    PGconn *handle = NULL;
    int status = 1;
    ...
    if ((handle = librdf_storage_postgresql_get_handle(storage))) {
        ...
        if (context_node) {
            ...
            if (ctxt) {
                query = LIBRDF_MALLOC(
                char*, strlen(delete_context) + (20 * 2) + 1);
                if (query) {
 ...
 **/
static librdf_stream *
librdf_storage_postgresql_find_statements_in_context(librdf_storage *storage, librdf_statement *statement,
                                                     librdf_node *context_node) {
...
librdf_storage_postgresql_find_statements_with_options(librdf_storage *storage,
    ...
    *where = '\0';
    if (sos->is_literal_match)
        sprintf(joins, " FROM Literals AS L LEFT JOIN Statements"
    UINT64_T_FMT
    " as S ON L.ID=S.Object",
            context->model);
    else
    sprintf(joins, " FROM Statements"
    UINT64_T_FMT
    " AS S", context->model);
    ...
    if (statement && subject) {
        ...
        if (librdf_storage_postgresql_find_statements_in_context_augment_query(&query,
                                                                               " SubjectR.URI AS SuR, SubjectB.Name AS SuB")) {
            librdf_storage_postgresql_find_statements_in_context_finished((void *) sos);
            return NULL;
        }
    ...
    if (statement && object) {
        if (!sos->is_literal_match) {
            ...
            if (librdf_storage_postgresql_find_statements_in_context_augment_query(&query,
                                                                                   " ObjectR.URI AS ObR, ObjectB.Name AS ObB, ObjectL.Value AS ObV, ObjectL.Language AS ObL, ObjectL.Datatype AS ObD")) {
                librdf_storage_postgresql_find_statements_in_context_finished((void *) sos);
                return NULL;
            }
    ...
    if (context_node) {
        ...
        if (librdf_storage_postgresql_find_statements_in_context_augment_query(&query,
                                                                               " ContextR.URI AS CoR, ContextB.Name AS CoB, ContextL.Value AS CoV, ContextL.Language AS CoL, ContextL.Datatype AS CoD")) {
            librdf_storage_postgresql_find_statements_in_context_finished((void *) sos);
            return NULL;
        }
    ...
    sos->current_rowno = 0;
    sos->row = LIBRDF_CALLOC(
    char**, LIBRDF_GOOD_CAST(size_t, PQnfields(sos->results) + 1), sizeof(char *));
    if (!sos->row) {
        librdf_storage_postgresql_find_statements_in_context_finished((void *) sos);
        return NULL;
    }
...
librdf_storage_postgresql_find_statements_in_context_augment_query(char **query, const char *addition) {
    ...
    LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(query,
    char, 1);
    LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(addition,
    char, 1);

    /* Augment existing query, returning 0 on success. */
    newquery = LIBRDF_MALLOC(
    char*, strlen(*query) + strlen(addition) + 1);
    if (!newquery)
        return 1;
...
librdf_storage_postgresql_find_statements_in_context_end_of_stream(void *context) {
    ...
    LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(context,
    void, 1);
...
librdf_storage_postgresql_find_statements_in_context_finished(void *context) {
    ...
    LIBRDF_ASSERT_OBJECT_POINTER_RETURN(context,
    void);
...
LEFT JOIN Literals AS L ON S.Context=L.ID";
    ...
    /* Construct query */
    query = LIBRDF_MALLOC(
    char*, strlen(select_contexts) + 21);
    if (!query) {
        librdf_storage_postgresql_get_contexts_finished((void *) gccontext);
        return NULL;
    }
    ...
    gccontext->row = LIBRDF_CALLOC(
    char**, LIBRDF_GOOD_CAST(size_t, PQnfields(gccontext->results) + 1), sizeof(char *));
    if (!gccontext->row) {
        librdf_storage_postgresql_get_contexts_finished((void *) gccontext);
        return NULL;
    }
