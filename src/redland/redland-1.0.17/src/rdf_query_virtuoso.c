...
librdf_query_virtuoso_init(librdf_query *query, const char *name,
    ...
    query_string_copy = LIBRDF_MALLOC(
    unsigned char*, len + 1);
...
librdf_query_virtuoso_execute(librdf_query *query, librdf_model *model) {
    ...
    cmd = LIBRDF_MALLOC(
    char*, pref_len + query_string_len + 1);
    if (!cmd) {
        goto error;
    }
    ...
    if (context->numCols > 0) {
        context->colNames = LIBRDF_CALLOC(
        char**,
        ...
        for (icol = 1; icol <= context->numCols; icol++) {
            ...
            context->colNames[icol - 1] = LIBRDF_MALLOC(
            char*, snamelen + 1);
 ...
 **/
static int
librdf_query_virtuoso_results_get_bindings(librdf_query_results *query_results, const char ***names,
                                           librdf_node **values) {
    librdf_query *query = query_results->query;
    librdf_query_virtuoso_context *context;
    short col;
