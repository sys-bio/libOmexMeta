...
main(int argc, char *argv[]) {
            ...
            case 'o':
                if (optarg) {
                    if (!strcmp(optarg, "help")) {
                        ...
                        fprintf(stderr,
                                "%s: WARNING: The deprecated 'simple' output type has been replaced with N-Triples\n",
                                program);
                        ...
                        if (!output_serializer) {
                            fprintf(stderr,
                                    "%s: invalid argument `%s' for `" HELP_ARG(o, output) "'\nTry '%s " HELP_ARG(o,
                                                                                                                 output) " help' for a list of valid serializers\n",
                        ...
                        if (librdf_query_results_formats_check(world, optarg, NULL, NULL))
                            ...
                            fprintf(stderr,
                                    "%s: invalid argument `%s' for `" HELP_ARG(r, results) "'\nTry '%s " HELP_ARG(r,
                                                                                                                  results) " help' for a list of valid query result formats\n",
                                    program, optarg, program);
                            usage = 1;
                        }
                    }
                ...
                if (optarg) {
                    ...
                    if (!librdf_get_storage_factory(world, optarg)) {
                        fprintf(stderr, "%s: invalid argument `%s' for `" HELP_ARG(s, storage) "'\nTry '%s " HELP_ARG(s,
                                                                                                                      storage) " help' for a list of valid storages\n",
                                program, optarg, program);
                        usage = 1;
                    } else
                        storage_name = optarg;
    ...
    switch (type) {
        ...
        case CMD_PARSE_STREAM:
                ...
                if (argc >= 4 && argv[3]) {
                    if (librdf_heuristic_is_blank_node(argv[3]))
                        target = librdf_new_node_from_blank_identifier(world,
                                                                       (const unsigned char *) librdf_heuristic_get_blank_node(
                                                                               argv[3]));
                    else
            ...
            if (rc) {
                librdf_uri *error_count_uri = librdf_new_uri(world,
                                                             (const unsigned char *) LIBRDF_PARSER_FEATURE_ERROR_COUNT);
                librdf_uri *warning_count_uri = librdf_new_uri(world,
                                                               (const unsigned char *) LIBRDF_PARSER_FEATURE_WARNING_COUNT);
                ...
                case CMD_QUERY:
                    ...
                    if (type == CMD_FIND || type == CMD_MATCH) {
                        if (argc == 4 || type == CMD_MATCH) {
                            if (argc == 4) {
                                if (librdf_heuristic_is_blank_node(argv[3]))
                                    context_node = librdf_new_node_from_blank_identifier(world,
                                                                                         (const unsigned char *) librdf_heuristic_get_blank_node(
                                                                                                 argv[3]));
                                else
                            ...
                            if (type == CMD_MATCH) {
                                ...
                                stream = librdf_model_find_statements_with_options(model, partial_statement,
                                                                                   context_node, match_options);
                                librdf_free_hash(match_options);
                            } else {
                                stream = librdf_model_find_statements_in_context(model, partial_statement,
                                                                                 context_node);
                            }
