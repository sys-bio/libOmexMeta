...
semsim::SemsimXmlAssistant::SemsimXmlAssistant(std::string xml, std::string base, int metaid_num_digits) :
        xml_(std::move(xml)), metaid_base(std::move(base)), metaid_num_digits_(metaid_num_digits) {
}

const std::vector<std::string> &semsim::SemsimXmlAssistant::getValidElements() const {
    return valid_elements_;
}

void semsim::SemsimXmlAssistant::generateMetaId(std::vector<std::string> &seen_metaids, long count, const MetaID &metaid_gen,
                                                std::string &id) {
...
void semsim::SemsimXmlAssistant::addMetaIdsRecursion(xmlNode *a_node, std::vector<std::string> &seen_metaids) {
        ...
        if (cur_node->type == XML_ELEMENT_NODE) {
            ...
            if (std::find(getValidElements().begin(), getValidElements().end(),
                ...
                if (!has_meta_id) {
                    ...
                    std::string id;
                    semsim::SemsimXmlAssistant::generateMetaId(seen_metaids, count, metaId, id);
...
std::pair<std::string, std::vector<std::string>> semsim::SemsimXmlAssistant::addMetaIds() {
    LIBXML_TEST_VERSION;
...
semsim::XmlAssistantPtr semsim::XmlAssistantFactory::generate(const std::string &xml, SemsimXmlType type) {
    switch (type) {
        case semsim::SEMSIM_TYPE_SBML: {
            semsim::SBMLAssistant sbmlAssistant(xml);
            return std::make_unique<semsim::SBMLAssistant>(sbmlAssistant);
        }
        case semsim::SEMSIM_TYPE_CELLML: {
            semsim::CellMLAssistant cellMlAssistant(xml);
            return std::make_unique<semsim::CellMLAssistant>(cellMlAssistant);
        }
        case semsim::SEMSIM_TYPE_OTHER: {
            semsim::SemsimXmlAssistant xmlAssistant(xml);
            return std::make_unique<semsim::SemsimXmlAssistant>(xmlAssistant);
        }
        default:
